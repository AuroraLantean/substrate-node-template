...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			info!("Hello World from offchain workers!");
			...
			debug!("Current block: {:?} (parent hash: {:?})", block_number, parent_hash);
	...
	/// Defines the block when next unsigned transaction will be accepted. To prevent spam of unsigned (and unpayed!) transactions on the network, we only allow one transaction every `T::UnsignedInterval` blocks. This storage entry defines when new transaction is going to be accepted.
	#[pallet::storage]
...
impl<T: Config> Pallet<T> {
	...
	fn fetch_price_and_send_signed() -> Result<(), &'static str> {
		...
		for (acc, res) in &results {
			match res {
				Ok(()) => info!("[{:?}] Submitted price of {} cents", acc.id, price),
	...
	fn fetch_price() -> Result<u32, http::Error> {
		...
		let price = match Self::parse_price(body_str) {
			Some(price) => Ok(price),
			None => {
				warn!("Unable to extract price from the response: {:?}", body_str);
				Err(http::Error::Unknown)
			},
		}?;

		warn!("Got price: {} cents", price);
	...
	fn add_price(maybe_who: Option<T::AccountId>, price: u32) {
		info!("Adding to the average: {}", price);
		...
		info!("Current average price is: {}", average);
