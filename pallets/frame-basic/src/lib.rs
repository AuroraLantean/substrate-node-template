...
// A custom weight calculator tailored for the dispatch call `set_balance()`. This actually examines
...
//
// The rules of `WeightForSetBalance` are as follows:
// - The final weight of each dispatch is calculated as the argument of the call multiplied by the
//   parameter given to the `WeightForSetBalance`'s constructor.
...
struct WeightForSetBalance<T: pallet_balances::Config>(BalanceOf<T>);

impl<T: pallet_balances::Config> WeighData<(&BalanceOf<T>,)> for WeightForSetBalance<T> {
...
impl<T: pallet_balances::Config> ClassifyDispatch<(&BalanceOf<T>,)> for WeightForSetBalance<T> {
...
impl<T: pallet_balances::Config> PaysFee<(&BalanceOf<T>,)> for WeightForSetBalance<T> {
...
pub mod pallet {
	...
	//
	// `fn foo(origin: OriginFor<T>, accounts: Accounts, baz: Baz) -> DispatchResultWithPostInfo { ... }`
	//
	...
	impl<T: Config> Pallet<T> {
		...
		/// world.
		// This just increases the value of `TotalBalance` by `increase_by`.
		//
		...
		pub fn add(origin: OriginFor<T>, increase_by: T::Balance) -> DispatchResult {
			// This is a public call, so we ensure that the origin is some signed account.
			let _sender = ensure_signed(origin)?;

			// Read the value of total_balance from storage.
			...
			// let new_balance = total_balance.map_or(increase_by, |total_balance| total_balance + increase_by);

			// Put the new value into storage.
			// <TotalBalance<T>>::put(new_balance);
			// Will also work with a reference:
			// <TotalBalance<T>>::put(&new_balance);

			// Here's the new one of read and then modify the value.
			<TotalBalance<T>>::mutate(|total_balance| {
				// Using `saturating_add` instead of a regular `+` to avoid overflowing
				let new_balance =
					total_balance.map_or(increase_by, |d| d.saturating_add(increase_by));
				*total_balance = Some(new_balance);
			});

			// Let's deposit an event to let the outside world know this happened.
			Self::deposit_event(Event::AccumulateBalance { balance: increase_by });

			// All good, no refund.
			Ok(())
		}

		/// A privileged call; in this case it resets our total_balance value to something new.
		...
		//
		// The weight for this extrinsic we use our own weight object `WeightForSetBalance` to
		// determine its weight
		#[pallet::call_index(1)]
		#[pallet::weight(WeightForSetBalance::<T>(<BalanceOf<T>>::from(100u32)))]
		pub fn set_balance(
			origin: OriginFor<T>,
			...
			// Put the new value into storage.
			<TotalBalance<T>>::put(new_value);

			Self::deposit_event(Event::SetBalance { balance: new_value });
	...
	pub enum Event<T: Config> {
		// Just a normal `enum`, here's a total_balance event to ensure it compiles.
		/// TotalBalance event, just here so there's a generic type that's used.
		AccumulateBalance {
			balance: BalanceOf<T>,
		},
		SetBalance {
	...
	pub(super) type TotalBalance<T: Config> = StorageValue<_, T::Balance>;

	// A map that has enumerable entries.
	#[pallet::storage]
	#[pallet::getter(fn accounts)]
	pub(super) type Accounts<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, T::Balance>;
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
		fn build(&self) {
			<TotalBalance<T>>::put(&self.total_balance);
			for (a, b) in &self.accounts {
				<Accounts<T>>::insert(a, b);
			}
			<Foo<T>>::put(&self.foo);
		}
	}
...
/// A simple signed extension that checks for the `set_balance` call. In that case, it increases the
...
pub struct WatchBalance<T: Config + Send + Sync>(PhantomData<T>);

impl<T: Config + Send + Sync> sp_std::fmt::Debug for WatchBalance<T> {
	fn fmt(&self, f: &mut sp_std::fmt::Formatter) -> sp_std::fmt::Result {
		write!(f, "WatchBalance")
	}
}

impl<T: Config + Send + Sync> SignedExtension for WatchBalance<T>
where
	<T as frame_system::Config>::RuntimeCall: IsSubType<Call<T>>,
{
	const IDENTIFIER: &'static str = "WatchBalance";
	...
	fn validate(
		...
		// check for `set_balance`
		match call.is_sub_type() {
			Some(Call::set_balance { .. }) => {
				sp_runtime::print("set_balance was received.");
